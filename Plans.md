...
| 9.1a | Fuzzer pipeline proof: `tools/fuzz/gen-demo.mjs` — seeded, reproducible synthetic vanilla demos (v1.9 header, bounded ticcmds: forwardmove/sidemove ∈ legal ranges, angleturn, buttons; 0x80 terminator), wrapped as single-lump PWADs; run the SAME 10 seeds on webdoom (wasm) AND nat-doom (native), compare per-tic state hashes | 10 seeded synthetic games run on both engines with per-tic hashes bit-identical (or a divergence honestly reported with its seed); generator + runner committed | 8.2 | cc:TODO |
| 9.1b | Scale + CI: ≥1000 seeded games through the 9.1a pipeline (batched), summary report; fast tier (~20 games) wired into run-tests.sh; full tier documented as release gate | ≥1000 games, 0 divergences (or divergence root-caused as a FINDING); CI-able fast tier green in run-tests.sh | 9.1a | cc:TODO |
