...
| 11.1a | **Rung 1 — hosted freestanding core**: a build of engine/core + a minimal platform layer that assumes ONLY (a) a memory region (static arena allocator behind I_ZoneBase), (b) a byte-out (putchar-class) for the per-tic hash stream, (c) the WAD handed over as a preloaded memory blob by a thin host shim. No SDL, no file I/O inside core's view, no floating libc surface beyond what core actually links. Extends tools/native-sanitize (its web.h/perf.h shims are the FINDING-6-proven 5-header surface). Dumps per-tic state hashes over the byte-out | the freestanding-style build boots headless, runs a demo, and streams per-tic hashes that MATCH nat-doom's for the same demo; every libc symbol it still imports is enumerated in a committed list (the honest platform surface) | 8.1 | cc:TODO |
| 11.1b | **Rung 2 — QEMU bare-metal ELF** (Cortex-M or RISC-V, no OS): the 11.1a core linked against a real bare-metal shim (linker script, startup, UART byte-out, WAD baked via objcopy), booted under qemu-system | the ELF boots in QEMU with no OS and streams per-tic hashes over the emulated UART; documented even if partial | 11.1a | cc:TODO |
