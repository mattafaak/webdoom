...
| 6.5 | **FINDING-9 (carved out of 7.3)**: `magic-data.md` — the only PUBLISHED doc — is outside doc-drift.mjs's scope, because the drift index is built from claims-index.md whose locators all point at engine-archaeology.md. Two public errors have now slipped through it (invuln 242→241 caught by FINDING-1 only by luck; luma 92→91 sat wrong for a whole phase after the internal fix landed). Bring the public doc under the gate: give its figures claim locators so a wrong number there FAILS the build | editing any figure in magic-data.md to a wrong value makes verify-all fail at that claim (drift-proved, same as 6.3); no published figure lacks a locator | 6.3 | cc:TODO |
