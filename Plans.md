...
| 9.2a | Coverage harness: a `--coverage` (gcov, no ASan) variant of tools/native-sanitize; run the 13 golden demos + ≥50 fuzz seeds through it; commit the harness + a machine-readable per-function/branch coverage report + a human summary (exercised %, top un-exercised files) | harness + report committed and regenerable by one script; report distinguishes function vs branch coverage; fuzz corpus demonstrably ADDS coverage over the demos alone (delta shown) | 9.1a | cc:TODO |
| 9.2b | Negative-space classification: every un-exercised engine/core path from 9.2a's report → understood-by-inference (with the doc section that covers it) or unknown (flagged as a finding) | zero 'unknown un-exercised' paths remain unclassified; classification committed alongside the report | 9.2a | cc:TODO |
