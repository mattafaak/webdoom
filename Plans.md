...
| 11.1b | **Rung 2 — QEMU bare-metal ELF** (Cortex-M or RISC-V, no OS): the 11.1a core linked against a real bare-metal shim (linker script, startup, UART byte-out, WAD baked via objcopy), booted under qemu-system | the ELF boots in QEMU with no OS and streams per-tic hashes over the emulated UART; documented even if partial | 11.1a | cc:BLOCKED(env) — lead scouted the host: **no qemu-system-\* and no bare-metal cross-gcc** (arm-none-eabi-gcc / riscv*-elf-gcc all absent). Cannot compile OR run a bare-metal ELF here; writing untested bare-metal C would violate the no-unverified-code rule. Needs: a cross toolchain + qemu-system-{arm,riscv}. Rung 1 (11.1a) already validated the platform contract 13/13, so partial-progress goal is met; this rung is deferred to an env that has the toolchain |
