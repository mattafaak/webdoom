...
{
    events[eventhead] = *ev;
    eventhead = (eventhead+1)&(MAXEVENTS-1);    // webdoom: was unsequenced UB
}
...
{
    // webdoom: WADs come from the in-heap registry (JS registered them
    // before main). Filenames keep their 1993 meanings.
    if ( W_WebFileExists ("doom2f.wad") )
    {
	gamemode = commercial;
	language = french;
    ...
    if ( W_WebFileExists ("doom2.wad") )
    {
	gamemode = commercial;
	D_AddFile ("doom2.wad");
	return;
    }
    if ( W_WebFileExists ("plutonia.wad") )
    {
	gamemode = commercial;
	D_AddFile ("plutonia.wad");
	return;
    }
    if ( W_WebFileExists ("tnt.wad") )
    {
	gamemode = commercial;
	D_AddFile ("tnt.wad");
	return;
    }
    if ( W_WebFileExists ("doomu.wad") )
    {
	gamemode = retail;
	D_AddFile ("doomu.wad");
	return;
    }
    if ( W_WebFileExists ("doom.wad") )
    {
	gamemode = registered;
	D_AddFile ("doom.wad");
	return;
    }
    if ( W_WebFileExists ("doom1.wad") )
    {
	gamemode = shareware;
	D_AddFile ("doom1.wad");
	return;
