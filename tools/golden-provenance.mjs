...
// WHY THIS EXISTS
// ---------------
// The project's most expensive recorded lesson (Plans-refinement-complete.md:80)
...
export function recordReason(argv) {
    ...
    if (sh('git status --porcelain') !== '' && !reason) {
        ...
        console.error('  (Plans-refinement-complete.md:80 — a regold once encoded a real bug).');
        process.exit(2);
    }
    return reason;
}
