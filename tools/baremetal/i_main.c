...
{
    int wad_len =
        (int) (_binary_obj_bm_wad_bin_end - _binary_obj_bm_wad_bin_start);

    // (1) Announce.
    bm_puts ("BM-DOOM-BOOT\n");

    // Disable stdio buffering so all printf output is immediately written to
    // UART. Without this, newlib may buffer output and we lose visibility into
    // hangs.
    setvbuf (stdout, NULL, _IONBF, 0);
    setvbuf (stderr, NULL, _IONBF, 0);

    // (2) Register baked WAD blob with files.c.
    fs_register_wad (BM_WAD_NAME, (byte*) _binary_obj_bm_wad_bin_start,
                     wad_len);
