...
static void scan_bsd_rand(void){
    ...
    for (uint64_t seed64 = 0; seed64 < 0x100000000ull; seed64++) {
        ...
        for (int i = 0; i < 4 && ok; i++) {
            ...
            if ((uint8_t)((t & 0x7fffffff) % 256) != canon_rnd[i]) ok = 0;
        }
        if (!ok) continue;
        t = (uint32_t)seed64; ok = 1;
        for (int i = 0; i < 256 && ok; i++) {
            t = t * 1103515245u + 12345u;
            if ((uint8_t)((t & 0x7fffffff) % 256) != canon_rnd[i]) ok = 0;
        }
        if (ok) { printf("*** MATCH bsd-rand seed=%u\n", (uint32_t)seed64); hits++; }
    }
    if (!hits) printf("no match: 4.3BSD rand low-byte (full 2^32)\n");
}
...
static int try_random(uint32_t seed, int glibc_init, int warmup){
    ...
    if (glibc_init) {
        ...
        for (int i = 1; i < 31; i++) {
            ...
            if (w < 0) w += 2147483647;
            st[i] = (uint32_t)w; prev = (int32_t)w;
        }
    } else {
        st[0] = seed;
        for (int i = 1; i < 31; i++)
    ...
    for (int k = 0; k < warmup + 256; k++) {
        ...
        f = (f + 1) % 31; r = (r + 1) % 31;
        if (k >= warmup)
            if ((uint8_t)(out % 256) != canon_rnd[k - warmup]) return 0;
    }
    return 1;
}
...
int main(void){
    ...
    struct { uint32_t lo, hi; const char* what; } R[] = {
        {0, 1u<<24, "small"},
        ...
        for (unsigned ri = 0; ri < 2; ri++) {
            ...
            for (uint64_t s = R[ri].lo; s < R[ri].hi; s++)
                if (try_random((uint32_t)s, g, w ? 310 : 0)) {
                    printf("*** MATCH random() %s init, warmup %d, seed=%llu\n",
                           g?"glibc":"bsd", w?310:0, (unsigned long long)s);
                    hits++;
                }
            if (!hits) printf("no match: random() %s init warmup=%d range=%s\n",
