...
typedef struct { const char* name; uint32_t mul, add; int shift; uint32_t mask; int mod256; } lcg_t;
    ...
    for (unsigned g = 0; g < sizeof(GENS)/sizeof(GENS[0]); g++) {
        ...
        for (uint64_t seed64 = 0; seed64 < 0x100000000ull; seed64++) {
            ...
            for (int i = 0; i < 4 && ok; i++) {
                ...
                uint8_t b = G->mod256 ? (uint8_t)(out % 256) : (uint8_t)(out & 255);
                if (b != canon_rnd[i]) ok = 0;
            }
            if (!ok) continue;
            // full verify
            t = s; ok = 1;
            ...
            if (ok) { found = s; hits++; printf("*** MATCH %s seed=%u (0x%x)\n", G->name, s, s); }
        }
        if (!hits) printf("no match: %s\n", G->name);
    }
    return 0;
}
