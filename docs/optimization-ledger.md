...
| **predicted Δinstr/tic** | **bsp-side (conservative FLOOR, two bsp sources):** R_DrawColumn ≈ 70% of bsp per perf.md §Q1. Using perf.md §13.1b bsp = 494,338 → 70% = ~346K; halved by low-detail → **~173K instr/tic** (≈14% of whole, doom.wad p50). Using cycle-attribution.json direct mean-of-p50s bsp = 558,834 → 70% = ~391K; halved → **~196K instr/tic** (≈16% of whole). Range: ~173K–196K instr/tic bsp-side savings. **planes-side (ADDITIONAL, UNMEASURED):** `r_main.c:734` sets `spanfunc = R_DrawSpanLow` when `detailshift=1`. `R_DrawSpanLow` performs ONE `ds_source/ds_colormap` lookup per TWO output pixels (writes `*dest++` twice per iteration, halving loop count). Additional savings ≈ ½ × 402K = **~200K instr/tic** — UNMEASURED pending 14.2b's before/after cycle-floor run. **Combined ceiling (bsp + planes, pre-measurement):** ~373–396K instr/tic ≈ **~30–32% of whole-program** (doom.wad p50). NOTE: actual split requires 14.2b sub-stage profiling to confirm; DoD for 14.2b should capture both bsp and planes deltas. |
