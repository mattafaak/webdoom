...
## 12b. Render variants (14.2b; 20.3a–d retired)

One variant ships beside the vanilla path, and it is a runtime switch, not a
build: low-detail (`web_set_detail`), the column path `R_DrawColumnLow` /
`R_DrawSpanLow` of §7.6. It has its own golden family and its own leg.

| variant | build dir | what changes | golden family | suite leg |
|---------|-----------|--------------|---------------|-----------|
| *(none)* | `build/` | the vanilla path | `*-render.json` (13) | `render-goldens` |
| low-detail via `--low-detail` | `build/` | low-detail column path (`web_set_detail`) | `*-render-low.json` (13) | `render-low` |

The four FastDoom compile-time toggles of tasks 20.3a–d (fake-flat, potato
columns, status-bar redraw skip, differential blit) were retired in round 10
(2026-09-16). No player could reach them, two were never measured or
measured a net loss, and they cost four build trees, eleven suite legs, 26
golden files and eight `#line` directives in the core. Their removal was
proven the way their presence was: `build/doom.wasm` stayed byte-identical
(`b65732eb1be202690e861c36d7849afe`, 355,883 bytes) with the blocks and the
directives gone, so the directives had been inert all along. Their
measurements and kill rules stay in `docs/optimization-ledger.md` §20.3a–d
as history.

The sim-invariance claim outlives them: `sim-freelook` and `sim-invariants`
run the renderer with the render-side options active against the sim goldens.
