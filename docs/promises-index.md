...
quantitative claims across the five archaeology docs. **That count is
deliberately not repeated here** — `claims-index-check.mjs` prints the manifest
total and the index row total on every run, and this paragraph is the wrong
place to keep a second copy. It had one: "154 in the manifest, 204 index rows",
against a real 157 and 207, inside a sentence claiming both figures were
asserted against the tables. Neither was; rule 5 matches only `/(\d+)
promises/`. A stale number is bad, and a stale number wearing a claim that it
cannot go stale is worse.

This index covers the 48 promises that live outside that corpus.
...
**48 promises. Counts are asserted against the table by `tools/archaeology/promises-index-check.mjs`, not typed here — the header and the summary used to disagree (5/8/15 against 5/10/13), and the checker itself matched only three of the four Parts until this pass.**
...
|----|--------|---------|-------------|
| rme-001 | README:5 | "293 KB of wasm" | **GATED** — `readme-001` + `size-004` in claims.json; `doc-drift.mjs` README_HINTS and `size-ledger.mjs` size-004a/b both fail if README diverges. The two ids state the same fact and `claims-index-check.mjs` asserts they agree (they had drifted, 348 vs 349, task 24.2). |
| rme-002 | README:7–9 | "Runs in stock Chrome / Edge / Firefox" | **GATED** — Chromium by 19 CDP legs; Firefox by `firefox-smoke` (UA + JS + /api/wads) and `firefox-frame`, which drives Firefox to a real rendered DOOM frame and MEASURES it. [Why](#rme-002) |
| rme-003 | README:10–11 | "Uncapped framerate with 35 Hz-exact game logic (Crispy-style interpolation; \"vanilla mode\" toggle under OPTIONS on the launcher menu)" | **GATED** — the F8 panel is gone: SMOOTH RENDERING is a row on the launcher's OPTIONS screen, and `browser-options-test` drives it (reaching the screen, the value in force under hostile localStorage, and Reset defaults restoring it). [Why](#rme-003) |
| rme-004 | README:12–13 | "rebindable keys, analog twin-stick gamepad" | **GATED** — the REBIND half is gated: `tools/browser-options-test.mjs` (leg `browser-options`, 34 assertions) drives the CONTROLS screen through capture, Escape-cancels, the conflict swap, a partial stored bind map and Reset defaults, and closes with the round trip the old F8 test could not make — the key bound on OPTIONS is pressed in a running level and the engine must receive `DK.UP`. [Why](#rme-004) |
| rme-005 | README:26–27 | "the second load is served from that cache, single player works offline" | **GATED** — offline SP boot by `browser-offline` (boots from the SW cache with the network down) and `check-sw-precache` (the SHELL list both ways); the warm load by `load-budget`, a REGRESSION gate against a baseline committed per host, which a host without one SKIPs by name rather than passing. [Why](#rme-005) |
| rme-006 | README:18–20 | "measured < 1 ms/tick on the weakest network host" (fire) | **EVIDENCED** — perf.md §fire: wbox 0.0722 ms/tick (best-of-10 × 2000, 2026-07-16); ~14× under budget. Node microbench (not browser). Committed in perf.md; not CI-automated (requires JS bench). |
| rme-007 | README:95–97 | "cross-validated tic-for-tic against an instrumented Chocolate Doom... 44,580 tics identical" | **EVIDENCED** — re-verified 2026-07-17 (task 12.5): SDL2 packages present on CachyOS host; `bash tools/build-choco-reference.sh` RC=0; `node tools/demo-test.mjs --cross` 13/13 demos PASS, 44,580 tics identical. Expected value in claims.json md-tic-001=44580 unchanged. Not gateable in CI (requires external binary + WADs); run on demand via the two commands above. |
| rme-008 | README:24–27 | "Server carries the WAD library (Ultimate Doom, Doom II, Final Doom, SIGIL, Master Levels, NRFTL, Chex Quest)" | **GATED** — `smoke-pwad` leg (`tools/smoke-pwad-test.mjs`): **24 of 24** library WADs the demo goldens do NOT cover are booted and rendered, with four assertions each — the sim advances (≥ FRAMES/4 tics), the framebuffer is non-black (≥ 10,000/64,000 px), it CHANGES over the run, and for a PWAD it differs from the base IWAD warped to the same map. [Why](#rme-008) |
| rme-009 | README:52 | "`webdoom.service` is a ready systemd unit" | **GATED** — `service-file` leg (`tools/service-check.sh`), quick tier, **15 assertions**: systemd's own parser with its OUTPUT graded as well as its status (warnings exit 0), the nine directives a ready unit must carry, ExecStart's program executable and its script present in this repo, WorkingDirectory's basename matching the checkout, and DOOM_HOST/DOOM_PORT agreeing with `server/serve.js`'s own defaults. Red-proofed on five arms — a typo'd `ExecStartt=`, a removed `Restart=`, a port that disagrees with the server, an absent ExecStart script and a broken `[Install]` — each rc=1 naming a different failure. `systemd-analyze verify` alone was NOT made the gate: on a good unit it exits 0 and prints nothing, so a bare wrapper could not tell a sound unit from a dead check. **What it does not check, stated rather than implied:** whether the unit BOOTS — that needs root and a live systemd. |
| rme-010 | README (removed) | "T07 menu-nav is a pre-existing timing flake on some CI hosts — ~1/3 pass rate" | **RESOLVED** — the promise this tracked is gone. T07 was fixed in 9ed9671 (3-attempt retry of the MP-open action, assertion unweakened, 20/20 on a fresh profile); the original cause was /tmp exhaustion from orphaned Chrome, not this codebase. README no longer claims a ~1/3 pass rate (24.3). Note `check-state-machine` verifies edge COVERAGE by token grep, not that each assertion runs. |
| rme-011 | README:14–15 | "Authentic audio: DMX PCM sfx via WebAudio, music through an emulated OPL2 (Nuked OPL3) playing the IWAD's own GENMIDI bank" | **GATED** — `opl-mode` renders the engine's own OPL2 output and compares it byte-for-byte; `browser-music-worklet` proves the synth runs in the AudioWorklet on a secure origin with the main-thread stage empty; `browser-music-fallback` and `browser-insecure` prove the `BufferSink` path an insecure origin takes; `wad-content-fuzz` feeds hostile GENMIDI and MUS lumps. The three nouns the bullet names are each covered: DMX PCM sfx (`i_sound.c`, exercised by every browser leg that boots), the Nuked OPL3 core (`opl-mode`), and the IWAD's own GENMIDI rather than a bundled bank (`formats.md` §GENMIDI, `fmt-*` claims). **Added in round 12**: this bullet had no row at all, so tenet 6's completeness half had nothing to say about it. |
| rme-012 | README:16–17 | "1–4 players: instant single player; arcade lobby for network play (join order = color: Green/Indigo/Brown/Red — nothing to type)" | **GATED, with one word called out** — the player counts by `net-2p` and `net-4p` (real wasm clients through the relay) and `browser-net`; the lobby, the join-order colouring and the no-typing claim by `browser-lobby` and `browser-join`, and by `state-machine`, which checks the lobby's enumerated edges against the code. **"instant" is not gated and is not claimed to be**: it means "no install step", which is true by construction and is not a timing promise; `rme-005` covers the load-time sense of the word and deliberately dropped it there. **Added in round 12.** |
| rme-013 | README:21–23 | "Deterministic-lockstep netcode over a server tic relay; verified by a headless harness that compares per-tic gamestate hashes across real clients" | **GATED** — this is a description of `tools/net-test.mjs`, which is what `net-2p` and `net-4p` run: two and four real wasm engines through the real server, comparing `_web_state_hash()` per tic and failing on the first divergence. `join-coop`, `join-dm`, `churn` and `edge` cover drop-in re-simulation to the frontier; `spectate` compares a receive-only observer against player 0 at every common tic. The adversarial direction is `net-fuzz` and `hostile-server`. **Added in round 12**: the bullet that describes the project's central correctness claim was the one with no row. |
...
Across all 48: **31 gated, 10 evidenced, 5 flagged, 1 resolved, 1 ungateable.**
...
**48 promises in the table.** Counts are asserted against it by
...
Part sizes: A=13, B=9, C=10, D=16. Whole table: 48 promises — 10 evidenced,
5 flagged, 31 gated, 1 resolved, 1 ungateable. No PARTIAL rows remain: round 11
gated the uncapped-framerate half of rme-003 and the analog half of rme-004.

These figures are NOT typed from a reading: `promises-index-check.mjs`
recomputes every one of them from the rows and prints them on each run, which
is the only reason they can be trusted. The block that stood here did not —
it said 12 flagged and 18 gated over a 44-row total, all four numbers wrong in
the direction that flatters, because rows were re-dispositioned one at a time
and nothing added them up again.
