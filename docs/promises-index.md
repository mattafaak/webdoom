...
|----|--------|---------|-------------|
| rme-001 | README:5 | "292 KB of wasm" | **GATED** — `readme-001` + `size-004` in claims.json; `doc-drift.mjs` README_HINTS and `size-ledger.mjs` size-004a/b both fail if README diverges. The two ids state the same fact and `claims-index-check.mjs` asserts they agree (they had drifted, 348 vs 349, task 24.2). |
